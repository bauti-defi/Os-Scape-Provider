...
import org.osscape.api.wrappers.Widget;
import org.osscape.api.wrappers.WidgetChild;

public class Widgets {

	public static Widget[] get() {
		Object[][] widgets = (Object[][]) Engine.getReflectionEngine().getFieldHookValue("Widgets", null);
		if (widgets == null)
			return new Widget[0];
	...
	public static WidgetChild getWidgetWithText(String text) {
		...
		for (int parentIndex = 0; parentIndex < widgets.length; parentIndex++) {
			...
			for (int childIndex = 0; childIndex < children.length; childIndex++) {
				...
				if (child != null) {
					String widgetText = (String) Engine.getReflectionEngine().getFieldHookValue("Text", child);
	...
	public static WidgetChild getWidgetWithAction(String text) {
		...
		for (int parentIndex = 0; parentIndex < widgets.length; parentIndex++) {
			...
			for (int childIndex = 0; childIndex < children.length; childIndex++) {
				Object child = children[childIndex];
				if (child != null) {
					String[] widgetText = (String[]) Engine.getReflectionEngine().getFieldHookValue("WidgetActions", child);
	...
	public static boolean doContinue() {
		WidgetChild widgetChild = getWidgetWithText("Click here to continue");
		return widgetChild != null && widgetChild.isVisible() && widgetChild.click();
	}
}
